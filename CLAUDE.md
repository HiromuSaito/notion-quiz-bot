...
### 現状の課題
現在は問題と解答をまとめてLINEに送信しているが、インタラクティブな体験（問題表示 → 回答 → 正誤判定 → 解説 → 次の問題）を実現したい。

### 検討した選択肢

1. **LINE双方向対応**
   - メリット: 追加インフラ不要、既存の延長
   - デメリット: LINE Messaging APIの制約、UIの柔軟性が低い

2. **S3 + 署名付きURL**
   - メリット: サーバーレス、低コスト
   - デメリット: 静的なHTMLのみ、双方向性に限界

3. **Webアプリ（推奨）**
   - メリット: UIの柔軟性が高い、将来の拡張（穴埋め、自由記述）に対応可能
   - デメリット: インフラが必要（ただしVercel等で無料運用可能）

### 推奨アプローチ: Webベース + LINE通知のハイブリッド

将来の拡張（穴埋め、自由記述、回答記録など）を考慮すると、Webアプリとして構築し、LINEは通知・リンク送信に特化させるハイブリッドアプローチが最適。
...
│  1. Notionからコンテンツ取得                                      │
│  2. Geminiでクイズ生成                                           │
│  3. クイズデータをDBに保存（Supabase等）                          │
│  4. LINEにクイズページへのリンクを送信                            │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
